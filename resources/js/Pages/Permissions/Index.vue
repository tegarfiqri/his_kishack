...
const onSortChange = () => {
    ...
    if (sortBy.value) {
        // router.get('/permissions', { sortBy: sortBy.value, sortDesc: sortDesc.value }, { preserveState: true });
        repopulateData();
    }
};
...
const onSearch = () => {
    // router.get('/permissions', { search: search.value }, { preserveState: true });
    search.value = search.value;
    repopulateData();
};

const repopulateData = () => {
    router.get('/permissions', { sortBy: sortBy.value, sortDesc: sortDesc.value, search: search.value, page: pageNumber.value }, { preserveState: true });
}

const onDelete = (item) => {
    if (confirm('Are you sure you want to delete this permission?')) {
        router.delete(route('permissions.destroy', item.id), { preserveScroll: true, onFinish: () => repopulateData() });
    }
}
    ...
    <AuthenticatedLayout>
        <div>
            ...
            <div class="flex justify-between">
                ...
                <CreateButton v-if="checkPermission('Create Permissions')" :route="route('permissions.create')">
                    Add
                    Permission</CreateButton>
            </div>
